// app/class/[id]/pyqs-year/[subject]/[year]/layout.jsx
"use client";
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", height: "90vh" }}>
        <aside style={{ width: "300px", borderRight: "1px solid #ccc", padding: "20px", overflowY: "auto" }}>
          <h2>📚 Papers - {subject} {year}</h2>
          <ul style={{ listStyle: "none", padding: 0 }}>
            {pdfList?.map((file, idx) => {
              const filename = file.url.split("/").pop().replace(".pdf", "");
              return (
                <li key={idx} style={{ marginBottom: "10px" }}>
                  <Link href={`/class/${id}/pyqs-year/${subject}/${year}/${filename}`}>
                    {file.title}
                  </Link>
                </li>
              );
